...
int minimum_depth_of_tree(std::unique_ptr<Tree_Node>& T) {
    if(!T) {
        return 0;
    }
    ...
    if(!T->left) {
        return 1 + minimum_depth_of_tree(T->right);
    }

    // If right subtree is null, you must go down left
    if(!T->right) {
        return 1 + minimum_depth_of_tree(T->left);
    }

    return 1 + std::min(minimum_depth_of_tree(T->left), minimum_depth_of_tree(T->right));
}

TEST(Minimum_depth_of_tree, handles_various_cases) {
    ...
    EXPECT_EQ(minimum_depth_of_tree(root), 2);
    EXPECT_EQ(minimum_depth_of_tree(root2), 2);
}