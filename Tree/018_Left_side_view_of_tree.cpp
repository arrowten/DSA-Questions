...
std::vector<int> left_side_view_of_tree(std::unique_ptr<Tree_Node>& T) {
    if(!T) {
        return {};
    }
...
TEST(Left_side_view_of_tree, handles_various_cases) {
    ...
    const std::vector<int> result = {1, 2, 4};
    const std::vector<int> result2 = {1, 2, 5};

    EXPECT_EQ(left_side_view_of_tree(root), result);
    EXPECT_EQ(left_side_view_of_tree(root2), result2);
}