...
TEST(Inverted_trees_check, handles_various_cases) {
    ...
    root2->right = std::make_unique<Tree_Node>(8);
    root2->left = std::make_unique<Tree_Node>(12);
    root2->left->left = std::make_unique<Tree_Node>(14);
    root2->right->right = std::make_unique<Tree_Node>(16);
    root2->right->right->left = std::make_unique<Tree_Node>(20);
    
    EXPECT_TRUE(inverted_trees_check(root, root2));
}